...
OUTPUT_DIR  = Path("./models/logreg_svm")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

RANDOM_SEED = 42  # Same seed as friend's RF for fair comparison
...
def evaluate_model(model_name, y_true, y_pred, y_proba, cm_save_path, feat_imp_path=None,
                   feat_names=None, feat_values=None, feat_label="Importance"):
    """
    ...
    if feat_imp_path and feat_names is not None and feat_values is not None:
        ...
        ax.set_title(f"Top {top_n} Features by {feat_label} – {model_name}",
...
# Top 5 for text file
top5_lr = list(zip(lr_coef_df["Feature"].head(5),
                   lr_coef_df["AbsCoefficient"].head(5)))

save_results_txt(
    path       = OUTPUT_DIR / "model_results_lr.txt",
    model_name = "Logistic Regression",
    config_dict= {"solver": "lbfgs", "max_iter": 1000, "C": 1.0,
...
print(f"   Top feature: {lr_coef_df.iloc[0]['Feature']} "
      f"(|coef| = {lr_coef_df.iloc[0]['AbsCoefficient']:.4f})")
print(f"   Positive coef (→ NLOS): "
      f"{(lr_model.coef_[0] > 0).sum()} features")
print(f"   Negative coef (→ LOS) : "
      f"{(lr_model.coef_[0] < 0).sum()} features")
print()
...
svm_coefs = np.mean(
    [est.estimator.coef_[0] for est in svm_model.calibrated_classifiers_],
    axis=0
)
...
top5_svm = list(zip(svm_coef_df["Feature"].head(5),
                    svm_coef_df["AbsCoefficient"].head(5)))

save_results_txt(
    path       = OUTPUT_DIR / "model_results_svm.txt",
    model_name = "SVM (LinearSVC + Calibration)",
    config_dict= {"C": 1.0, "max_iter": 2000, "class_weight": "balanced",
                  "calibration_cv": 3, "random_state": RANDOM_SEED},
...
print(f"   Top feature : {svm_coef_df.iloc[0]['Feature']} "
...
for _, row in combined.head(10).iterrows():
    print(f"{int(row['AvgRank']):<5} {row['Feature']:<20} "
