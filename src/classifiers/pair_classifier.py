...
OUTPUT_DIR = Path("./models/pair_classifier")
...
# ==============================================================================
print()
print("Step 3: Training XGBoost pair classifier...")
print("-" * 60)
...
# ==============================================================================
print("Step 5: Plotting confusion matrices...")
...
for ax, y_pred_m, name in [
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", ax=ax,
                ...
                linewidths=0.5)
    ax.set_title(f"Pair Classifier — {name}", fontsize=13,
...
# STEP 6: ROC CURVES
# ==============================================================================
print("Step 6: Plotting ROC curves...")
...
for y_proba_m, name in [
    ...
    ax.plot(fpr, tpr, color=COLORS[name], linewidth=2.2,
            label=f"{name} (AUC = {auc:.4f})")

ax.set_xlabel("False Positive Rate\n(NLOS+NLOS misclassified as LOS+NLOS)", fontsize=11)
ax.set_ylabel("True Positive Rate\n(NLOS+NLOS correctly identified)", fontsize=11)
ax.set_title("Pair Classifier ROC Curve\nLOS+NLOS vs NLOS+NLOS",
...
ax.set_title("Pair Classifier — Top 20 Feature Importance (Random Forest)\n"
...
ax.set_title("Pair Classifier — Feature Importance by Category (RF)",
             fontsize=12, fontweight="bold")
ax.grid(axis="y", alpha=0.3)
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "pair_importance_by_category.png", dpi=300, bbox_inches="tight")
print("✓ Saved: pair_importance_by_category.png")
plt.close()

# ==============================================================================
# STEP 8: SAVE PREDICTIONS & MODELS
# ==============================================================================
print()
print("Step 8: Saving predictions and models...")
...
# ==============================================================================
# STEP 9: SAVE RESULTS TEXT
# ==============================================================================
metrics_df = pd.DataFrame([metrics_rf, metrics_xgb]).set_index("Model")
...
with open(OUTPUT_DIR / "pair_results.txt", "w") as f:
    ...
    for name, met in [("Random Forest", metrics_rf), ("XGBoost", metrics_xgb)]:
        f.write(f"{name}:\n")
...
print(f"  {'-'*52}")
for name, met in [("Random Forest", metrics_rf), ("XGBoost", metrics_xgb)]:
    print(f"  {name:<20} {met['Accuracy']:>10.4f} {met['F1-Score']:>10.4f} {met['ROC-AUC']:>10.4f}")
print()
print("KEY INSIGHT:")
